...
@section('title', 'Mekanik - Bengkel POS')
...
<div class="mb-6 flex justify-between items-center">
    ...
    <a href="{{ route('admin.mechanics.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
...
<div class="grid grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-lg shadow p-4">
        ...
        <p class="text-2xl font-bold">{{ $mechanics->total() }}</p>
    </div>
    <div class="bg-white rounded-lg shadow p-4">
        <p class="text-sm text-gray-500">Aktif</p>
        <p class="text-2xl font-bold text-green-600">{{ $mechanics->where('is_active', true)->count() }}</p>
    </div>
    <div class="bg-white rounded-lg shadow p-4">
        <p class="text-sm text-gray-500">Total Gaji Pending</p>
        <p class="text-2xl font-bold text-yellow-600">Rp {{ number_format($mechanics->sum('pending_salary'), 0, ',', '.') }}</p>
    </div>
    <div class="bg-white rounded-lg shadow p-4">
        <p class="text-sm text-gray-500">Total Sudah Dibayar</p>
        <p class="text-2xl font-bold text-blue-600">Rp {{ number_format($mechanics->sum('total_earnings'), 0, ',', '.') }}</p>
    </div>
</div>
...
<div class="bg-white rounded-lg shadow p-4 mb-6">
    <form method="GET" class="flex gap-4">
        <input type="text" name="search" placeholder="Cari mekanik (nama, telepon, spesialisasi)..." value="{{ request('search') }}"
    ...
    <table class="w-full">
        ...
        <tbody class="divide-y">
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-3">
                    <button onclick="toggleSalary({{ $mechanic->id }})" class="text-gray-500 hover:text-gray-700">
                        <i id="toggle-icon-{{ $mechanic->id }}" class="fas fa-chevron-right transition-transform"></i>
                    </button>
                </td>
                <td class="px-4 py-3">
                    <p class="font-medium">{{ $mechanic->name }}</p>
                    @if($mechanic->address)
                    <p class="text-xs text-gray-500">{{ Str::limit($mechanic->address, 40) }}</p>
                    @endif
                </td>
                <td class="px-4 py-3 text-sm">
                    <p>{{ $mechanic->phone ?? '-' }}</p>
                    <p class="text-gray-400 text-xs">{{ $mechanic->email ?? '' }}</p>
                </td>
                <td class="px-4 py-3 text-center text-sm">{{ $mechanic->specialization ?? '-' }}</td>
                <td class="px-4 py-3 text-center">
                    <span class="px-2 py-1 bg-purple-100 text-purple-800 rounded text-sm">{{ $mechanic->commission_rate ?? 0 }}%</span>
                </td>
                <td class="px-4 py-3 text-center">
                ...
                <td class="px-4 py-3 text-right">
                    @if($pendingSalary > 0)
                    <span class="text-yellow-600 font-medium">Rp {{ number_format($pendingSalary, 0, ',', '.') }}</span>
                    @else
                ...
                <td class="px-4 py-3 text-center">
                    <div class="flex justify-center gap-2">
                        <a href="{{ route('admin.mechanics.edit', $mechanic) }}" class="text-yellow-600 hover:text-yellow-800" title="Edit">
                            <i class="fas fa-edit"></i>
                        </a>
                        <button onclick="showCommissionModal({{ $mechanic->id }}, '{{ $mechanic->name }}', {{ $mechanic->commission_rate ?? 0 }})" 
                            class="text-purple-600 hover:text-purple-800" title="Set Komisi">
                            <i class="fas fa-percent"></i>
                        </button>
                        @if($pendingSalary > 0)
                        <button onclick="showPaySalaryModal({{ $mechanic->id }}, '{{ $mechanic->name }}', {{ $pendingSalary }})" 
                            class="text-green-600 hover:text-green-800" title="Bayar Gaji">
                            <i class="fas fa-money-bill-wave"></i>
                        </button>
                        @endif
                        <form action="{{ route('admin.mechanics.destroy', $mechanic) }}" method="POST" class="inline"
            ...
            <!-- Salary Detail Row -->
            <tr id="salary-row-{{ $mechanic->id }}" class="hidden bg-gray-50">
                <td colspan="8" class="px-4 py-4">
                    <div class="ml-8">
                        <div class="flex justify-between items-center mb-3">
                            ...
                            <div class="flex gap-2">
                                <div class="text-sm">
                                    ...
                                    <span class="font-medium text-green-600">Rp {{ number_format($paidSalary, 0, ',', '.') }}</span>
                                </div>
                            </div>
                        </div>
                        @if($mechanic->salaries->count() > 0)
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y">
                                @foreach($mechanic->salaries as $salary)
                                <tr class="{{ $salary->status === 'pending' ? 'bg-yellow-50' : '' }}">
                                    <td class="py-2">
                                        {{ $salary->period_start->format('d/m/Y') }} - {{ $salary->period_end->format('d/m/Y') }}
                                    </td>
                                    <td class="py-2 text-right">Rp {{ number_format($salary->service_amount, 0, ',', '.') }}</td>
                                    <td class="py-2 text-right">{{ $salary->commission_rate }}%</td>
                                    <td class="py-2 text-right font-medium">Rp {{ number_format($salary->commission_amount, 0, ',', '.') }}</td>
                                    ...
                                    <td class="py-2 text-center">
                                        ...
                                        <form action="{{ route('admin.mechanics.salary.paid', $mechanic) }}" method="POST" class="inline">
                                            @csrf
