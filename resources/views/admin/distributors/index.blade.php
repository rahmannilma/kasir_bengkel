...
@section('title', 'Distributor - Bengkel POS')
...
<div class="bg-white rounded-lg shadow p-4 mb-6">
    <form method="GET" class="flex gap-4">
        <input type="text" name="search" placeholder="Cari distributor, perusahaan..." value="{{ request('search') }}"
    ...
    <table class="w-full">
        ...
        <tbody class="divide-y">
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-3">
                    <button onclick="toggleNotes({{ $distributor->id }})" class="text-gray-500 hover:text-gray-700">
                        <i id="toggle-icon-{{ $distributor->id }}" class="fas fa-chevron-right transition-transform"></i>
                    </button>
                </td>
                <td class="px-4 py-3 font-medium">{{ $distributor->name }}</td>
                <td class="px-4 py-3 text-sm text-gray-500">{{ $distributor->company ?? '-' }}</td>
                <td class="px-4 py-3 text-sm">
                    <p>{{ $distributor->phone ?? '-' }}</p>
                    <p class="text-gray-400">{{ $distributor->email ?? '' }}</p>
                </td>
                <td class="px-4 py-3 text-center">
                    <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded text-sm">
                        {{ $distributor->categories_count }}x
                    </span>
                </td>
                <td class="px-4 py-3 text-center">
                    <div class="flex justify-center gap-1">
                        @if($totalNotes > 0)
                        <span class="px-2 py-1 bg-gray-100 text-gray-800 rounded text-xs" title="Total Nota">{{ $totalNotes }}</span>
                        @endif
                        @if($unpaidNotes > 0)
                        <span class="px-2 py-1 bg-red-100 text-red-800 rounded text-xs" title="Belum Lunas">{{ $unpaidNotes }}</span>
                        @endif
                        @if($overdueNotes > 0)
                        <span class="px-2 py-1 bg-red-200 text-red-900 rounded text-xs font-bold" title="Jatuh Tempo">{{ $overdueNotes }}</span>
                        @endif
                    </div>
                </td>
                <td class="px-4 py-3 text-right">
                    @if($remainingAmount > 0)
                    <span class="text-red-600 font-medium">Rp {{ number_format($remainingAmount, 0, ',', '.') }}</span>
                    @else
                ...
                <td class="px-4 py-3 text-center">
                    <div class="flex justify-center gap-2">
                        <button onclick="editDistributor({{ $distributor->id }}, '{{ $distributor->name }}', '{{ $distributor->phone }}', '{{ $distributor->email }}', '{{ $distributor->address }}', '{{ $distributor->company }}')"
                ...
                <td colspan="8" class="px-4 py-4">
                    <div class="ml-8">
                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y">
                                @foreach($distributor->notes as $note)
                                <tr class="{{ $note->is_overdue ? 'bg-red-50' : '' }}">
                                    <td class="py-2 font-medium">{{ $note->note_number }}</td>
                                    <td class="py-2">{{ $note->date->format('d/m/Y') }}</td>
                                    <td class="py-2 {{ $note->is_overdue ? 'text-red-600 font-bold' : '' }}">
                                        {{ $note->due_date->format('d/m/Y') }}
                                        @if($note->is_overdue)
                                            <span class="text-xs">(Terlambat)</span>
                                        @endif
                                    </td>
                                    <td class="py-2 text-right">Rp {{ number_format($note->total_amount, 0, ',', '.') }}</td>
                                    <td class="py-2 text-right text-green-600">Rp {{ number_format($note->paid_amount, 0, ',', '.') }}</td>
                                    <td class="py-2 text-right font-medium">Rp {{ number_format($note->remaining_amount, 0, ',', '.') }}</td>
                                    ...
                                    <td class="py-2 text-center">
                                        <div class="flex justify-center gap-2">
                                            ...
                                            @endif
                                            <a href="{{ route('admin.distributor-notes.edit', $note) }}" class="text-yellow-600 hover:text-yellow-800 text-xs">
                                                Edit
                                            </a>
...
function showDistributorModal() {
    ...
    document.getElementById('distributor-form').action = '{{ route('admin.distributors.store') }}';
...
function showNoteModal(distributorId, distributorName) {
    ...
    document.getElementById('note-form').action = '{{ route('admin.distributor-notes.store') }}';
